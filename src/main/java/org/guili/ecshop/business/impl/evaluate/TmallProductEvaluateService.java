 ...
 *
 */
public class TmallProductEvaluateService implements IProductEvaluateService {
	
	private static Logger logger=Logger.getLogger(TmallProductEvaluateService.class);
	...
	public double evaluateCalculate(String url, ModelMap modelMap,List<TmallAnalyzeBean> tmallAnalyzeBeanList) {
		...
		if(taobaoTotalAllData!=null){
			...
			tmallAnalyzeBean.setBadAndNormalWeightSingle(badAndNormal+reEval/TmallProductEvaluateService.Per_Addition);
...
//		logger.info("总分："+result);
	...
	public void createTmallAnalyzeBean(TaobaoTotalAllData taobaoTotalAllData,TmallAnalyzeBean tmallAnalyzeBean){
		...
		int badAndNormalWeightSingle=taobaoTotalAllData.getData().getCount().getBad()
				+taobaoTotalAllData.getData().getCount().getNormal();
		int reEval=taobaoTotalAllData.getData().getCount().getAdditional();
		int badWeightSingle=badAndNormalWeightSingle+reEval/reEval/TmallProductEvaluateService.Per_Addition;
	...
	public double evaluateSingleRepeat(Map<String, Map<String, Object>> productEvaluate,TaobaoTotalAllData taobaoTotalAllData){
		...
		double score=0;
		//第一阶段
		if(count>TmallProductEvaluateService.One_stage && count<=TmallProductEvaluateService.Two_stage){
			twoMax=new Double(count*TmallProductEvaluateService.Twice_People_max_One_stage).intValue();
			threeMax=new Double(count*TmallProductEvaluateService.Three_People_max_One_stage).intValue();
			score=getUserScore(usermap
					,TmallProductEvaluateService.per_two_score_One_stage
					,TmallProductEvaluateService.per_three_score_One_stage
					,TmallProductEvaluateService.per_four_score_One_stage
					,min,twoMax,threeMax,TmallProductEvaluateService.FourMore_People_One_stage);
		}else if(count>TmallProductEvaluateService.Two_stage && count<=TmallProductEvaluateService.Three_stage){
			//第二阶段
			twoMax=new Double(count*TmallProductEvaluateService.Twice_People_max_Two_stage).intValue();
			threeMax=new Double(count*TmallProductEvaluateService.Three_People_max_Two_stage).intValue();
			score=getUserScore(usermap
					,TmallProductEvaluateService.per_two_score_Two_stage
					,TmallProductEvaluateService.per_three_score_Two_stage
					,TmallProductEvaluateService.per_four_score_Two_stage
					,min,twoMax,threeMax,TmallProductEvaluateService.FourMore_People_Two_stage);
		}else if(count>TmallProductEvaluateService.Three_stage){
			//第 三阶段
			twoMax=new Double(count*TmallProductEvaluateService.Twice_People_max_Three_stage).intValue();
			threeMax=new Double(count*TmallProductEvaluateService.Three_People_max_Three_stage).intValue();
			score=getUserScore(usermap
					,TmallProductEvaluateService.per_two_score_Three_stage
					,TmallProductEvaluateService.per_three_score_Three_stage
					,TmallProductEvaluateService.per_four_score_Three_stage
					,min,twoMax,threeMax,TmallProductEvaluateService.FourMore_People_Three_stage);
		}
		return score;
	}
	...
	private double getUserScore(Map<String, Object> usermap
		...
		double score=TmallProductEvaluateService.SINGEL_INFO_EVALUATE_Repeat_PROPORTION_SCORE;
	...
	private  TmallEvaluate analyzeProductUrl(String userNumid,String auctionNumId,int page){
		...
		String htmltext=CommonTools.requestUrl(EvaluateConfig.tmall_evaluate_url, "gbk", "item_id="+auctionNumId+"&ps="+TmallProductEvaluateService.PAGE_SIZE+"&p="+page);
		try {
...
//		//卖家整体的差评比重分数
	...
	private double calculateWeightScore(TaobaoTotalAllData taobaoTotalAllData){
		...
		logger.debug("中差评权重："+badWeight);
		//中差评
		if(badWeight>=TmallProductEvaluateService.TOTAL_BAD_PROPORTION_MAX){
			weightScore=0;
		}else if(badWeight<=TmallProductEvaluateService.TOTAL_BAD_PROPORTION_MIN){
			weightScore=TOTAL_INFO_BAD_PROPORTION_SCORE;
		}else{
			weightScore=TOTAL_INFO_BAD_PROPORTION_SCORE*((TmallProductEvaluateService.TOTAL_BAD_PROPORTION_MAX-badWeight)/5);
		}
		return weightScore;
	}
	/**
	...
	private double calculateTotalDescScore(TaobaoTotalAllData taobaoTotalAllData){
		...
		if(correspond<TmallProductEvaluateService.TOTAL_INFO_DESC_SCORE_MIN){
			prevDescScore=0;
		}else if(correspond>=TmallProductEvaluateService.TOTAL_INFO_DESC_SCORE_MAX){
			prevDescScore=5;
		}else{
			prevDescScore=((correspond-TmallProductEvaluateService.TOTAL_INFO_DESC_SCORE_MIN)
	...
	public double singleProductEvaluate(Object obj) {
		...
		TaobaoTotalAllData taobaoTotalAllData=(TaobaoTotalAllData)obj;
		int total=taobaoTotalAllData.getData().getCount().getTotal();
		if(total<=TmallProductEvaluateService.SINGEL_TOTAL_LIMIT){
			return 0;
		}
		//计算单个商品中差评权重
	...
	public double evaluateSingleWeight(TaobaoTotalAllData taobaoTotalAllData){
		...
		int badAndNormal=taobaoTotalAllData.getData().getCount().getBad()
				+taobaoTotalAllData.getData().getCount().getNormal();
		int reEval=taobaoTotalAllData.getData().getCount().getAdditional();
		double singleWeight=((new Double(badAndNormal)+reEval/TmallProductEvaluateService.Per_Addition)/total)*100;
		//不同评价总量，不同的计算
		if(total>TmallProductEvaluateService.Three_stage){
			singleWeightScore=this.getSingleStageScore(SINGEL_BAD_PROPORTION_MIN, SINGEL_BAD_PROPORTION_MAX_500, singleWeight);
			
		}else if(total>TmallProductEvaluateService.Two_stage && total<=TmallProductEvaluateService.Three_stage){
			singleWeightScore=this.getSingleStageScore(SINGEL_BAD_PROPORTION_MIN, SINGEL_BAD_PROPORTION_MAX_100, singleWeight);
		}else if(total<=TmallProductEvaluateService.Two_stage && total >TmallProductEvaluateService.One_stage){
	...
	public static void main(String[] args) {
		TmallProductEvaluateService tmallProductEvaluate=new TmallProductEvaluateService();
	...
	public void AnalyzeTmallBrand() {
		for(String brandurl:AnalyzeTmallList.getInstance().getTmallBrandList()){
			if(brandurl!=null && !brandurl.isEmpty()){
				...
				int outerpage=0;
				if(allbrandurls.size()%TmallProductEvaluateService.OUTERPAGESIZE==0){
					outerpage=allbrandurls.size()/TmallProductEvaluateService.OUTERPAGESIZE;
				}else{
					outerpage=allbrandurls.size()/TmallProductEvaluateService.OUTERPAGESIZE+1;
				}
				for(int i=0;i<outerpage;i++){
					...
					logger.debug("AnalyzeTmallBrand page:----->"+i);
